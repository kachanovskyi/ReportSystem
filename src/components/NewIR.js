...
class NewIR extends Component {
    ...
    componentWillUnmount() {
        const subTitle = document.getElementsByClassName("parent-sub-title")[0];
        if (document.getElementsByClassName("navbar navbar-default navbar-fixed-top")[0] && subTitle) {
            document.getElementsByClassName("navbar navbar-default navbar-fixed-top")[0].removeChild(subTitle);
        }

        if (document.getElementsByClassName("NewIR shifted")[0]) {
            document.getElementsByClassName("NewIR shifted")[0].classList.remove("shifted");
        }

        if (document.getElementsByClassName("navbar navbar-default no-overflow")[0]) {
            document.getElementsByClassName("navbar navbar-default no-overflow")[0].classList.remove("no-overflow");
        }
    ...
    updateHeaderWithSubtitle = () => {
        ...
        subT.innerHTML = this.state.subTitle;
        ...
        // fetch('./data1.json')
            ...
            .then((responseJson) => {
                ...
                for (let x in responseJson) {
                    ...
                    if (x.includes("amount") || x === "protocol") {
                    ...
                    } else if (x === "scientific_research" || x === "scientific_research_global") {
                        ...
                        if (ifNotEmptyArray(responseJson[x])) {
                            ...
                            if (x === "scientific_research") {
                                this.setState({
                                    theme_id: theme_ids
                                });
                            } else {
                                this.setState({
                                    theme_id_global: theme_ids
                                });
                            }
                        } else {
                ...
                this.loadScientificGlobalManager();
            })
            .catch((error) => {
                console.log(error);
    ...
    loadScientificGlobalManager = () => {
        ...
        // fetch('./scientificData.json')
            ...
            .then((responseJson) => {
                ...
                self.setState({themes_global: themes});
                $('.theme_select').filter(function (index) {
                    return $(this).attr("id") === "scientific_research_global";
                }).each(function (index) {
                    if (self.state.theme_id_global[index]) {
                        $(this).val(self.state.theme_id_global[index]);
                    } else {
                        $(this).val(1);
                    }
                });
    ...
    loadScientificManager = () => {
        ...
        // fetch('./scientificData.json')
            ...
            .then((responseJson) => {
                ...
                $('.theme_select').filter(function (index) {
                    return $(this).attr("id") === "scientific_research";
                }).each(function (index) {
                    ...
                    <Col xs={10} xsOffset={1} lg={8} lgOffset={2}>
                        <form onSubmit={this.formSubmitted} className="IR-form">
                            ...
                            <InputField id="scientific_research_global" themes={this.state.themes_global}
                                        data={this.state.data.scientific_research_global}
                                        info="Участь у науково-дослідній тематиці кафедри(підрозділу): шифр, назва НДР (науковий керівник). Держбюджетна тематика"/>

                            <InputField id="scientific_research" themes={this.state.themes}
                                        data={this.state.data.scientific_research}
                                        info="Тематика в межах робочого часу"/>

                            <InputField id="participation_in_grant" data={this.state.data.participation_in_grant}
                                        info="участь у виконанні індивідуальних або колективних грантів"
                                        addInfo="окрім грантів на поїздки"/>

                            <InputField id="scientific_internship" info="наукові стажування"
                                        data={this.state.data.scientific_internship}/>
                            ...
                            <InputField id="students_scientific_management"
                                        data={this.state.data.students_scientific_management}
